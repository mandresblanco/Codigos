...
def ejecutar_consultar_duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print("La duración promedio de las peliculas es", promedio)
...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    anio = int(input("ingrese el año a partir del cual desea buscar los estrenos"))
    estrenos = mod.encontrar_estrenos(p1, p2, p3, p4, p5, anio)
    print("Peliculas estrenadas despues del año", estrenos)
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print("Número de películas con clasificación 18+:", cantidad)
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        nuevo_dia= input("Ingrese el nuevo día (ejm: Lunes, Jueves , Viernes)")
        nueva_hora = int(input("Ingrese la nueva hora (formato 24h, ej: 2130): "))
        resp = input("¿Desea tener en cuenta sus preferencias de horario? (s/n): ").lower()
        usar_preferencias = resp == "s"
        
    fue_reagendada = mod.reagendar_pelicula(pelicula, nueva_hora, nuevo_dia, usar_preferencias, p1, p2, p3, p4, p5)

    if fue_reagendada:
            print("La película fue reagendada exitosamente.")
    else:
            print("No fue posible reagendar la película con esos datos.")
...
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        permiso = input("¿Cuenta con la autorización de los padres? (s/n): ").lower()
        tiene_permiso = permiso == "s"

        puede_invitar = mod.decidir_invitar(pelicula, edad, tiene_permiso)

        if puede_invitar:
            print("¡Puedes invitar a esta persona a ver la película!")
        else:
            print("No puedes invitar a esta persona bajo estas condiciones.")
